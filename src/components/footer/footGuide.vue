...
<template>
    <div id="foot_guide">
      ...
      <section class="guide_item" @click="gotoAddress({path: '/msite',query:{geohash}})">
        <template v-if="$route.path.indexOf('msite')!==-1">
          <svg class="icon_style">
            <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#msiteActive"></use>
          </svg>
          <span>外卖</span>
        </template>
        <template v-else>
      ...
      <section class="guide_item" @click="gotoAddress({path:'/search/geohash'})">
        <template v-if="$route.path.indexOf('search')!==-1">
          <svg class="icon_style">
            <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#findActive"></use>
          </svg>
          <span>搜索</span>
        </template>
        <template v-else>
      ...
      <section class="guide_item" @click="gotoAddress({path:'/search/order'})">
        <svg class="icon_style">
          <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="$route.path.indexOf('order')!==-1?'#orderActive':'#order'"></use>
        </svg>
        <span>订单</span>
      </section>
      <section class="guide_item" @click="gotoAddress({path:'/search/profile'})">
        <svg class="icon_style">
          <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="$route.path.indexOf('profile')!==-1?'#msiteActive':'#profile'"></use>
        </svg>
        <span>我的</span>
      </section>
    </div>
